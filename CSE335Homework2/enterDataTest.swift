    ...
    var body: some View {
        NavigationView{
            VStack(spacing:100) {
                ...
                HStack() {
                    DatePicker("Date", selection: $enterDate );
                ...
                Button(action: {
                    ...
                    else if (checkNumeric(givenString: enterBPSys) == false || checkNumeric(givenString: enterBPDia) == false || checkNumeric(givenString: enterWeight) == false || checkNumeric(givenString: enterSugarLevel) == false )
                    {
                        ...
                        dataController.add_day(date: enterDate, weight: (enterWeight as NSString).doubleValue, sugarLevel: (enterSugarLevel as NSString).doubleValue, bloodPressureDiastolic: (enterBPDia as NSString).doubleValue, bloodPressureSystolic: (enterBPSys as NSString).doubleValue)
